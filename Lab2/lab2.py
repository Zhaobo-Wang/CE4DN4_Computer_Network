...
class Server:
    ...
    def connection_handler(self, client):
        ...
        self.print_client_info(client,address_port[1])
    ...
    def print_client_info(self,client,port):
...
class Client:
    ...
    def connection_receive(self):
        try:
            # key
            key_bytes = self.socket.recv(Client.RECV_BUFFER_SIZE)
            key_str = key_bytes.decode(Server.MSG_ENCODING)
            #  print(key_str)
            if len(key_bytes) == 0:
                print("Closing server connection ... ")
                self.socket.close()
            ...
            if "contain 7 digits" in key_str:
                print(key_str)
                self.socket.close()
                sys.exit(1)                
            elif "No encryption" in key_str:
                print(key_str)
                self.socket.close()
                sys.exit(1)
            else:
